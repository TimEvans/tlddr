...
**Repo:** `/home/tim/Github/tl-ddr` · branch `main` · 136 tests passing · HTML extractor + benchmark tooling merged; full pipeline proven end-to-end on Chevron
...
`tlddr extract --source <dir> --out <dir>` routes each file by **signal type** to a format extractor — all emitting one uniform `ExtractedDoc` (full faithful content + page-level provenance + warnings) to `.tlddr/extracted/<id>.json`, the content store the rest of the pipeline reads from. PDF (pymupdf), DOCX (python-docx, prose+tables; **note: `pages=[]`, no page markers**), XLSX (openpyxl, `--- sheet ---` markers, truncated/bloated), KMZ (stdlib, identity-only), **HTML/iXBRL** (`tlddr/extract/html.py`, BeautifulSoup+lxml — see below). Image-only pages flagged for vision (no model call yet).
...
**B. Broaden to a new corpus.** Chevron 10-K is now **done** (full pipeline, above; the HTML-extractor blocker is resolved). Remaining sibling filings: **CONSOL S-4** and **Microsoft 10-K** — both `.htm`-native, so the HTML extractor handles them; each needs its own `--out`/`--work`/`--vault`/`--benchmark` paths (there is no "active vault"; select by path, mirror the `chevron/` layout). A small `--profile <name>` deriving all paths from one name is the clean addition if juggling corpora becomes routine. The Chevron run also produced a real 42-section annual-report `sections.json` (`chevron/work/sections.json`) as a template example.

**B-opt. Draft-stage retrieval optimization (new, from the Chevron benchmark).** The benchmark quantified that draft/verify **re-run retrieval per section over the same 126-page 10-K** (~26× overview re-reads + overlapping page reads) — the bulk of the ~2.7M draft+verify tokens. Separate retrieval from generation: precompute a page→section index (each drafter handed its pages) and/or cluster sections that share an evidence region. Est. ~60–70% draft-token cut on a large shared source. Brainstorm seed with options+data: `docs/superpowers/ideas/2026-07-02-draft-retrieval-optimization.md`.
...
4. **Environment:** Arch Linux, system Python 3.14, project venv at `.venv`. Run tests with `.venv/bin/pytest` (136 passing). Editable install (`pip install -e ".[dev]"`). Deps: pydantic, pymupdf, python-docx, openpyxl, pyyaml, **beautifulsoup4, lxml** (+ pytest).
5. **Re-create a working vault if needed:** `tlddr extract` over a corpus populates `<out>/extracted/`; follow `skills/understand` then `skills/draft`. The Chevron run is fully materialized under `chevron/{work,vault,report,benchmark}` (gitignored) — a worked example of the whole pipeline incl. a real 42-section `sections.json`. All run artifacts (`.tlddr/`, `chevron/`, `vault/`, `report/`) and all of `docs/test-reports/` are gitignored.
