...
**Repo:** `/home/tim/Github/tl-ddr` · branch `main` · 164 tests passing · HTML extractor + benchmark tooling + single `--output` base dir + answer loop (D6) merged; full pipeline proven end-to-end on Chevron and the review cycle now closes
...
Closes the last designed-not-built seam: the pipeline is no longer a one-shot forward pass. `tlddr answer-commit (--answers <f> | --triage <f>) --output <base>` (new top-level module `tlddr/answer.py`: `ingest_answers`, `build_worklist`, `question_identity`, `parse_triage_answers`) validates Reviewer answer records `{id, disposition, answer}` — `Disposition` (`revise`|`accept`) and `resolved` are new `Question` fields — against the question store, resolves matches, clears `blocking`, and writes a deduped `work/worklist.json`. **Routing by `raised_by`:** `understand` questions route to a node re-pass, `draft`/`verify` questions route to a section re-pass; a node re-tag's downstream section impact is surfaced to the Reviewer, never auto-drafted (AD2). **Verify dedup:** `draft_verify_commit` now suppresses verdicts whose identity matches an already-resolved question (`draft/verify.py: ingest_verdicts(..., suppress=...)`), so a re-verify never re-raises a settled question. **Open/Resolved triage:** `understand/render.py: render_triage` re-derives each node's display triage from its still-unresolved questions only (`_display_triage`) and splits questions into `## Open questions` (with `> answer:` slots) and `## Resolved questions` (disposition + recorded answer, no slot) — so de-escalation shows up without mutating stored triage. **Disclosed caveats:** `draft/assemble.py: render_sidecar` renders resolved+`accept` questions per section as "Disclosed caveats (accepted findings)"; resolved+`revise` questions are hidden (superseded by the re-pass). `assemble` warns when a re-pass target has cycled 3+ times (`work/repass_log.json`, B3). The interactive Reviewer procedure lives in **`skills/review`** (one open question at a time, grounded probable-answer + ranked interpretations, explicit sign-off before commit, walks the worklist, checks convergence); `skills/draft` and `skills/understand` each gained a short "Re-pass mode" note for consuming a `worklist.json` entry's `guidance`. Spec/plan: `docs/superpowers/specs/2026-07-03-answer-loop-design.md`, `.../plans/2026-07-03-answer-loop.md`. Proving run: the Chevron run's 19 open verify questions (`docs/chevron-run-status.md`) — a real model run, not a unit test.
...
**Recommended immediate first action (user's decision, 2026-07-03):** the answer loop (D6) is now built — **prove it** by running the interactive `skills/review` session + `tlddr answer-commit` over the Chevron run's 19 open verify questions (`docs/chevron-run-status.md`) to drive the first real re-pass cycle end-to-end. Then pick a direction above (A–E) to extend the system further.
