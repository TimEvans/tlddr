...
4. **Environment:** Arch Linux, system Python 3.14, project venv at `.venv`. Run tests with `.venv/bin/pytest` (164 passing). Editable install (`pip install -e ".[dev]"`). Deps: pydantic, pymupdf, python-docx, openpyxl, pyyaml, **beautifulsoup4, lxml** (+ pytest).
