...
**Repo:** `/home/tim/Github/tl-ddr` · branch `main` · 175 tests passing · HTML extractor + benchmark tooling + single `--output` base dir + answer loop (D6) + its post-smoke-test hardening merged; full pipeline proven end-to-end on Chevron and the review cycle now closes with a surgical, not full-section, re-pass
...
| **Answer loop (D6) spec + plan** | `docs/superpowers/specs/2026-07-03-answer-loop-design.md`, `docs/superpowers/plans/2026-07-03-answer-loop.md` |
| **Answer loop hardening — smoke-test findings + design + plan** | `docs/superpowers/2026-07-03-answer-loop-smoke-test-findings.md`, `docs/superpowers/specs/2026-07-03-answer-loop-hardening-design.md`, `docs/superpowers/plans/2026-07-03-answer-loop-hardening.md` |
...
| Python package | `tlddr/` (`tlddr/extract/`, `tlddr/understand/`, `tlddr/draft/`, `tlddr/answer.py`, `tlddr/bench.py`) — each subpackage has a `CLAUDE.md` index |
| Host-agent procedures (SKILL.md) | `skills/generate-sections`, `skills/understand`, `skills/draft`, `skills/draft-verify`, `skills/review` |
...
### Answer loop — D6 (merged, this session)
Closes the last designed-not-built seam: the pipeline is no longer a one-shot forward pass. `tlddr answer-commit (--answers <f> | --triage <f>) --output <base>` (new top-level module `tlddr/answer.py`: `ingest_answers`, `build_worklist`, `parse_triage_answers`) validates Reviewer answer records `{id, disposition, answer}` — `Disposition` (`revise`|`accept`) is the answer-record input vocabulary, mapped onto the question's lifecycle status — against the question store, resolves matches, and writes a deduped `work/worklist.json`. **Routing by `raised_by`:** `understand` questions route to a node re-pass, `draft`/`verify` questions route to a section re-pass; a node re-tag's downstream section impact is surfaced to the Reviewer, never auto-drafted (AD2). **Verify dedup:** `draft_verify_commit` suppresses verdicts for an already-resolved question (`draft/verify.py: ingest_verdicts(..., suppress_ids=...)`), so a re-verify never re-raises a settled question. **Open/Resolved triage:** `understand/render.py: render_triage` re-derives each node's display triage from its still-open questions only (`_display_triage`) and splits questions into `## Open questions` (with `> answer:` slots) and `## Resolved questions` (status + recorded answer, no slot) — so de-escalation shows up without mutating stored triage. **Disclosed caveats:** `draft/assemble.py: render_sidecar` renders `accepted`-status questions per section as "Disclosed caveats (accepted findings)"; `revise`-routed questions are hidden while their re-pass is pending or just applied (superseded by the re-pass). `assemble` warns when a re-pass target has cycled 3+ times (`work/repass_log.json`, B3). (The `resolved`/`disposition` `Question` fields and the `verify-{index}` question id this paragraph originally described were superseded by the post-smoke-test hardening below — `Question.status` and claim-id-keyed verify questions.) The interactive Reviewer procedure lives in **`skills/review`** (one open question at a time, grounded probable-answer + ranked interpretations, explicit sign-off before commit, walks the worklist, checks convergence); `skills/draft` and `skills/understand` each gained a short "Re-pass mode" note for consuming a `worklist.json` entry's `guidance`. Spec/plan: `docs/superpowers/specs/2026-07-03-answer-loop-design.md`, `.../plans/2026-07-03-answer-loop.md`. Proving run: the Chevron run's 19 open verify questions (`docs/chevron-run-status.md`) — a real model run, not a unit test.

### Answer loop hardening — post-smoke-test (merged, this session)
Driving the interactive `skills/review` session over the Chevron run's 19 open verify questions end-to-end proved the loop's plumbing sound but found its **prescribed re-pass mechanism harmful**: a full-section regeneration via `skills/draft` (what the D6 skills told the agent to do) churned claims the Reviewer never questioned and reopened already-accepted findings, confirmed by a throwaway blind-regeneration experiment. Several defects (unstable `verify-{index}` question ids colliding across re-passes, weak dedup) traced to one shared root: **claims had no durable identity**. Findings: `docs/superpowers/2026-07-03-answer-loop-smoke-test-findings.md`. Design + plan: `docs/superpowers/specs/2026-07-03-answer-loop-hardening-design.md`, `.../plans/2026-07-03-answer-loop-hardening.md`. Fixes, all merged:
- **`DraftClaim.id`** — a durable surrogate, content-hash-at-birth (`draft/claims.py: _claim_id(section_id, text)`), frozen once assigned (a same-section text collision gets a numeric suffix in `cli.py: draft_commit`).
- **`Question.status`** (new `QuestionStatus`: open/accepted/revise_pending/revise_applied) replaces the old `resolved`+`disposition` pair; `Disposition` is retained as the answer-record input vocabulary only (`ingest_answers` maps it onto `status`).
- **`draft/verify.py: ingest_verdicts`** now keys verdicts on `claim_id` instead of array index; a raised question's id is `verify-{claim_id}-{reason}`, which is the dedup key — stable across re-passes (an index-based id was not).
- **`draft-amend`** (new `tlddr/draft/amend.py: apply_amendments`, wired into `cli.py`) is now the sanctioned re-pass mechanism: a batch of `{claim_id, set_text?, add_pages?, set_support?, set_evidence?}` records, re-validated through the same grounding checks as `draft-commit`, drop-and-report on an unknown id or a failed re-validation — a **surgical** patch, not a full section re-draft.
- **Lifecycle flips:** `draft_commit`/`understand_commit` flip their own target's `revise_pending` questions to `revise_applied`; `draft_amend` flips by `claim_id`. `assemble` warns (warn-only, never blocks) on any question still `revise_pending` with no re-pass applied.
- `skills/draft-verify` (claim-id verdicts), `skills/draft`, and `skills/review` (re-pass mode now prescribes `draft-amend`, explicitly not a full re-draft) updated to match.

---

## NEXT — candidate directions (all five stages + the answer loop exist; pick one to start)

All five stages exist and are proven forward (extract → understand → draft → verify → assemble), **and the answer loop (D6) now closes the cycle** (see "Answer loop — D6" above) — a Reviewer's signed-off answer routes back through a scoped re-pass to convergence, so the pipeline is no longer a one-shot forward pass. Brainstorm the chosen direction first (superpowers workflow).
...
4. **Environment:** Arch Linux, system Python 3.14, project venv at `.venv`. Run tests with `.venv/bin/pytest` (175 passing). Editable install (`pip install -e ".[dev]"`). Deps: pydantic, pymupdf, python-docx, openpyxl, pyyaml, **beautifulsoup4, lxml** (+ pytest).
...
**Recommended immediate first action (2026-07-03) — DONE:** the answer loop (D6) proving run (above: the interactive `skills/review` session + `tlddr answer-commit` over the Chevron run's 19 open verify questions) drove the first real re-pass cycle end-to-end, surfaced that the prescribed re-pass mechanism was harmful, and that hardening is now merged (see "Answer loop hardening" above). Next: pick a direction above (A–E) to extend the system further.
