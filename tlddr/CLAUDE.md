...
- `_apply_revises(work_dir, match) -> None` - flips a question's status from `revise_pending` to `revise_applied` when it matches the given predicate (used by `understand_commit`/`draft_commit` on their own target; by `draft_amend` on the amended claim ids)
...
- `answer_commit(answers_path, triage_path, work_dir, sections_path=None, vault_dir=None) -> None` - ingest reviewer answer records (from `--answers` JSON or parsed from a filled `--triage` _triage.md), set each matched question's `status` (`accepted`/`revise_pending`), write `work/worklist.json`, bump the repass log, print the worklist, and re-render the vault triage
