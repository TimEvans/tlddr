...
Write the curated list to `sections.json` as a JSON array in document order. Top-level entries use `{id, title}`; nested entries add `parent`; entries with body content add `guidance`.
